...
AlignerGlobalUkkonen::AlignerGlobalUkkonen(
    ...
    : AlignerGlobal(max_query_length, max_target_length, max_alignments,
      ...
      score_matrices_(), ukkonen_p_(100)
{
...
void AlignerGlobalUkkonen::run_alignment(
    ...
    int32_t num_alignments, sycl::queue *stream)
{
    ...
    {
        max_length_difference = std::max(
            max_length_difference, std::abs(sequence_lengths_h[2 * i] -
                                            sequence_lengths_h[2 * i + 1]));
    }
