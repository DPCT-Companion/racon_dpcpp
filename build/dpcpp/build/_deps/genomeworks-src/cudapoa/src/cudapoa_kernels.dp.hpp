...
#include "cudapoa_nw.dp.hpp"
#include "cudapoa_nw_banded.dp.hpp"
#include "cudapoa_nw_adaptive_banded.dp.hpp"
#include "cudapoa_topsort.dp.hpp"
#include "cudapoa_add_alignment.dp.hpp"
#include "cudapoa_generate_consensus.dp.hpp"
#include "cudapoa_generate_msa.dp.hpp"
                                  ...
                                  BandMode band_mode         = BandMode::full_band)
{
    ...
    int32_t warp_idx = item_ct1.get_local_id(2) / WARP_SIZE;
    int32_t lane_idx = item_ct1.get_local_id(2) % WARP_SIZE;
    int32_t window_idx = item_ct1.get_group(2) * nwindows_per_block + warp_idx;
    ...
    for (SizeT s = 1; s < num_sequences; s++)
    {
        ...
        /*
        DPCT1023:56: The DPC++ sub-group does not support mask options for
        sycl::select_from_group.
        */
        warp_error =
            sycl::select_from_group(item_ct1.get_sub_group(), warp_error, 0);
        if (warp_error)
        {
            return;
        }
        ...
        if (static_banded || adaptive_banded)
        {
            ...
            {
                alignment_length =
                    runNeedlemanWunschAdaptiveBanded<uint8_t, ScoreT, SizeT>(
                        nodes, sorted_poa, node_id_to_pos, sequence_lengths[0],
                        incoming_edge_count, incoming_edges,
                        outgoing_edge_count, sequence, seq_len, scores,
                        banded_score_matrix_size, alignment_graph,
                        alignment_read, static_band_width, gap_score,
                        mismatch_score, match_score, int8_t{0}, item_ct1);
                ...
                if (alignment_length < -2)
                {
                    ...
                    alignment_length =
                        runNeedlemanWunschAdaptiveBanded<uint8_t, ScoreT,
                                                         SizeT>(
                            nodes, sorted_poa, node_id_to_pos,
                            sequence_lengths[0], incoming_edge_count,
                            incoming_edges, outgoing_edge_count, sequence,
                            seq_len, scores, banded_score_matrix_size,
                            alignment_graph, alignment_read, static_band_width,
                            gap_score, mismatch_score, match_score,
                            static_cast<int8_t>(alignment_length), item_ct1);
                    item_ct1.barrier();
                }
            }
            else
            {
                alignment_length =
                    runNeedlemanWunschBanded<uint8_t, ScoreT, SizeT>(
                        nodes, sorted_poa, node_id_to_pos, sequence_lengths[0],
                        incoming_edge_count, incoming_edges,
                        outgoing_edge_count, sequence, seq_len, scores,
                        alignment_graph, alignment_read, static_band_width,
                        gap_score, mismatch_score, match_score, item_ct1);
                item_ct1.barrier();
            }
        }
        else
        {
            alignment_length = runNeedlemanWunsch<uint8_t, ScoreT, SizeT>(
                nodes, sorted_poa, node_id_to_pos, sequence_lengths[0],
                incoming_edge_count, incoming_edges, outgoing_edge_count,
                outgoing_edges, sequence, seq_len, scores, scores_width,
                alignment_graph, alignment_read, gap_score, mismatch_score,
                match_score, item_ct1);
            item_ct1.barrier();
        }
        ...
        /*
        DPCT1023:57: The DPC++ sub-group does not support mask options for
        sycl::select_from_group.
        */
        warp_error =
            sycl::select_from_group(item_ct1.get_sub_group(), warp_error, 0);
...
template <typename ScoreT, typename SizeT>
void generatePOA(
    genomeworks::cudapoa::OutputDetails *output_details_d,
    genomeworks::cudapoa::InputDetails<SizeT> *input_details_d,
    int32_t total_windows, sycl::queue *stream,
    ...
    ScoreT gap_score, ScoreT mismatch_score, ScoreT match_score,
    bool static_banded, bool adaptive_banded, uint32_t max_sequences_per_poa,
    int8_t output_mask, const BatchConfig &batch_size)
{
    ...
    /*
    DPCT1027:58: The call to cudaDeviceSetCacheConfig was replaced with 0
    ...
    stream->parallel_for(
        ...
        [=](sycl::nd_item<3> item_ct1) [[intel::reqd_sub_group_size(32)]] {
            generatePOAKernel<ScoreT, SizeT>(
                consensus_d, sequences_d, base_weights_d, sequence_lengths_d,
                window_details_d, total_windows, scores, alignment_graph,
                alignment_read, nodes, incoming_edges, incoming_edge_count,
                outgoing_edges, outgoing_edge_count, incoming_edge_w,
                outgoing_edge_w, sorted_poa, node_id_to_pos, node_alignments,
                node_alignment_count, sorted_poa_local_edge_count, node_marks,
                check_aligned_nodes, nodes_to_visit, node_coverage_counts,
                gap_score, mismatch_score, match_score, max_sequences_per_poa,
                sequence_begin_nodes_ids, outgoing_edges_coverage,
                outgoing_edges_coverage_count, max_nodes_per_graph,
                matrix_graph_dimension, matrix_seq_dimension,
                batch_size.max_consensus_size, item_ct1, TPB, adaptive_banded,
                static_banded, msa, batch_size.alignment_band_width);
        });
    /*
    ...
    */
    ...
    if (msa)
    {
        ...
        */
        stream->submit([&](sycl::handler &cgh) {
            sycl::accessor<SizeT, 0, sycl::access_mode::read_write,
                           sycl::access::target::local>
                msa_length_acc_ct1(cgh);

            cgh.parallel_for(
                sycl::nd_range<3>(
                    sycl::range<3>(1, 1, total_windows) *
                        sycl::range<3>(1, 1, max_sequences_per_poa),
                    sycl::range<3>(1, 1, max_sequences_per_poa)),
                [=](sycl::nd_item<3> item_ct1) {
                    generateMSAKernel<SizeT>(
                        nodes, consensus_d, window_details_d,
                        incoming_edge_count, incoming_edges,
                        outgoing_edge_count, outgoing_edges,
                        outgoing_edges_coverage, outgoing_edges_coverage_count,
                        node_id_to_msa_pos, sequence_begin_nodes_ids,
                        multiple_sequence_alignments, sequence_lengths_d,
                        sorted_poa, node_alignments, node_alignment_count,
                        max_sequences_per_poa, node_id_to_pos, node_marks,
                        check_aligned_nodes, nodes_to_visit,
                        max_nodes_per_graph, batch_size.max_consensus_size,
                        item_ct1, msa_length_acc_ct1.get_pointer());
                });
        });
        /*
    ...
    else
    {
        ...
        */
        stream->parallel_for(
            sycl::nd_range<3>(
                sycl::range<3>(1, 1, consensus_num_blocks) *
                    sycl::range<3>(1, 1, CUDAPOA_MAX_CONSENSUS_PER_BLOCK),
                sycl::range<3>(1, 1, CUDAPOA_MAX_CONSENSUS_PER_BLOCK)),
            [=](sycl::nd_item<3> item_ct1) {
                generateConsensusKernel<SizeT>(
                    consensus_d, coverage_d, sequence_lengths_d,
                    window_details_d, total_windows, nodes, incoming_edges,
                    incoming_edge_count, outgoing_edges, outgoing_edge_count,
                    incoming_edge_w, sorted_poa, node_id_to_pos,
                    node_alignments, node_alignment_count, consensus_scores,
                    consensus_predecessors, node_coverage_counts,
                    max_nodes_per_graph, batch_size.max_consensus_size,
                    item_ct1);
            });
        /*
