...
ScoreT* get_score_ptr_adaptive(ScoreT* scores, SizeT row, SizeT column, SizeT band_start, SizeT band_width)
{
...
template <typename ScoreT, typename SizeT>
void set_score_adaptive(ScoreT* scores, SizeT row, SizeT column, ScoreT value, SizeT band_start, SizeT band_width)
{
    SizeT col_offset;
    if (column == -1)
    {
...
SizeT get_band_start_for_row_adaptive(SizeT row, float gradient, SizeT band_width, SizeT band_shift, SizeT max_column)
{

    SizeT diagonal_index = SizeT(row * gradient);
    SizeT start_pos = sycl::max(0, (int)(diagonal_index - band_shift));
    if (max_column < start_pos + band_width)
    {
        start_pos =
            sycl::max(0, (int)(max_column - band_width + CELLS_PER_THREAD));
    }
...
template <typename ScoreT, typename SizeT>
ScoreT get_score_adaptive(ScoreT* scores, SizeT row, SizeT column,
                                     ...
                                     SizeT max_column, const ScoreT min_score_value)
{
    ...
    band_end = sycl::min(band_end, max_column);
...
template <typename ScoreT, typename SizeT>
ScoreT4<ScoreT> get_scores_adaptive(ScoreT* scores,
                                               SizeT row,
                                               SizeT column,
                                               SizeT band_width,
                                               SizeT band_shift,
                                               ...
                                               ScoreT4<ScoreT>& char_profile)
{
    ...
    else
    {
        ...
        score.s0 = sycl::max((int)(score4.s0 + char_profile.s0),
                             (int)(score4.s1 + gap_score));
        score.s1 = sycl::max((int)(score4.s1 + char_profile.s1),
                             (int)(score4.s2 + gap_score));
        score.s2 = sycl::max((int)(score4.s2 + char_profile.s2),
                             (int)(score4.s3 + gap_score));
        score.s3 = sycl::max((int)(score4.s3 + char_profile.s3),
                             (int)(score4_next.s0 + gap_score));
...
template <typename ScoreT, typename SizeT>
void initialize_band_adaptive(ScoreT* scores, SizeT row, ScoreT min_score_value, SizeT band_start, SizeT band_width,
                              sycl::nd_item<3> item_ct1)
{
    SizeT lane_idx = item_ct1.get_local_id(2) % WARP_SIZE;

    SizeT band_end = band_start + band_width;
    band_start = sycl::max(1, band_start);
                                   ...
                                   ScoreT gap_score)
{
    ...
    else
    {
        ...
        for (uint16_t p = 0; p < pred_count; p++)
        {
            ...
            penalty = sycl::max(
                penalty,
                get_score_adaptive(scores, pred_node_graph_pos, SizeT{-1},
                                   band_width, band_shift, gradient, max_column,
                                   min_score_value));
        }
                                     ...
                                     sycl::nd_item<3> item_ct1)
{
    ...
    int16_t lane_idx = item_ct1.get_local_id(2) % WARP_SIZE;
    int64_t score_index;
    ...
    {
        //                                                                            ad-hoc rule 1.b
        band_width = sycl::max(band_width,
                               cudautils::align<SizeT, CUDAPOA_MIN_BAND_WIDTH>(
                                   max_column * 0.08 * gradient));
    }
    if (gradient < 0.8) // ad-hoc rule 1.a
    {
        ...
        band_width = sycl::max(band_width,
                               cudautils::align<SizeT, CUDAPOA_MIN_BAND_WIDTH>(
                                   max_column * 0.1 / gradient));
    }

    // limit band-width for very large reads, ad-hoc rule 3
    band_width = sycl::min(band_width, 1536);
    ...
    for (SizeT graph_pos = 0; graph_pos < graph_count; graph_pos++)
    {
        ...
        initialize_band_adaptive(scores, score_gIdx, min_score_value,
                                 band_start, band_width, item_ct1);
        ...
        for (SizeT read_pos = lane_idx * CELLS_PER_THREAD + band_start; read_pos < band_start + band_width; read_pos += WARP_SIZE * CELLS_PER_THREAD)
        {
            ...
            for (uint16_t p = 1; p < pred_count; p++)
            {
                ...
                score.s0 = sycl::max(score.s0, scores_4.s0);
                score.s1 = sycl::max(score.s1, scores_4.s1);
                score.s2 = sycl::max(score.s2, scores_4.s2);
                score.s3 = sycl::max(score.s3, scores_4.s3);
            }
                    ...
                    loop))
            {
                ...
                /*
                DPCT1023:190: The DPC++ sub-group does not support mask options
                for sycl::shift_group_right.
                */
                ScoreT last_score = sycl::shift_group_right(
                    item_ct1.get_sub_group(), score.s3, 1);
                if (lane_idx == 0)
                {
                ...
                ScoreT tscore =
                    sycl::max((int)(score.s2 + gap_score), score.s3);
                if (tscore > score.s3)
                {
                    score.s3 = tscore;
                    loop     = true;
                ...
                tscore = sycl::max((int)(score.s1 + gap_score), score.s2);
                if (tscore > score.s2)
                {
                    score.s2 = tscore;
                    loop     = true;
                ...
                tscore = sycl::max((int)(score.s0 + gap_score), score.s1);
                if (tscore > score.s1)
                {
                    score.s1 = tscore;
                    loop     = true;
                ...
                tscore = sycl::max((int)(last_score + gap_score), score.s0);
                if (tscore > score.s0)
                {
                    score.s0 = tscore;
                    loop     = true;
            ...
            /*
            DPCT1023:191: The DPC++ sub-group does not support mask options for
            sycl::select_from_group.
            */
            first_element_prev_score = sycl::select_from_group(
                item_ct1.get_sub_group(), score.s3, WARP_SIZE - 1);
        ...
        while (!(i == 0 && j == 0) && loop_count < static_cast<int32_t>(read_length + graph_count + 2))
        {
            ...
            if (i != 0 && j != 0)
            {
                ...
                if (rerun == 0)
                {
                    ...
                    SizeT threshold =
                        sycl::max(1, (int)(max_column / 1024)); // ad-hoc rule 7
    ...
    /*
    DPCT1023:192: The DPC++ sub-group does not support mask options for
    sycl::select_from_group.
    */
    aligned_nodes =
        sycl::select_from_group(item_ct1.get_sub_group(), aligned_nodes, 0);
