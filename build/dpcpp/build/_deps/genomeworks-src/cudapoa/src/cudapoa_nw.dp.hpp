...
__dpct_inline__ ScoreT4<ScoreT> make_ScoreT4(ScoreT s0)
{
    ScoreT4<ScoreT> t;
    t.s0 = s0;
    t.s1 = s0;
...
template <typename SeqT, typename ScoreT, typename SizeT>
__dpct_inline__ ScoreT4<ScoreT>
computeScore(SizeT rIdx, SeqT4<SeqT> read4, SizeT gIdx, SeqT graph_base,
             uint16_t pred_count, SizeT pred_idx, SizeT *node_id_to_pos,
             SizeT *incoming_edges, ScoreT *scores, int32_t scores_width,
             ScoreT gap_score, ScoreT match_score, ScoreT mismatch_score)
{
    ...
    for (SizeT p = 1; p < pred_count; p++)
    {
        ...
        score.s0 = sycl::max((int)(score4.s0 + char_profile.s0),
                             sycl::max(score.s0, (int)(score4.s1 + gap_score)));

        score.s1 = sycl::max((int)(score4.s1 + char_profile.s1),
                             sycl::max(score.s1, (int)(score4.s2 + gap_score)));

        score.s2 = sycl::max((int)(score4.s2 + char_profile.s2),
                             sycl::max(score.s2, (int)(score4.s3 + gap_score)));

        score.s3 =
            sycl::max((int)(score4.s3 + char_profile.s3),
                      sycl::max(score.s3, (int)(score4_next.s0 + gap_score)));
    }
                       ...
                       sycl::nd_item<3> item_ct1)
{
        ...
        for (SizeT graph_pos = 0; graph_pos < graph_count; graph_pos++)
        {
            ...
            else
            {
                ...
                for (uint16_t p = 0; p < pred_count; p++)
                {
                    ...
                    penalty = sycl::max(penalty, scores[score_index]);
                }
                score_index         = static_cast<int64_t>(i) * static_cast<int64_t>(scores_width);
    ...
    for (SizeT graph_pos = 0; graph_pos < graph_count; graph_pos++)
    {
        ...
        for (SizeT read_pos = lane_idx * CPT; read_pos < readpos_bound; read_pos += WARP_SIZE * CPT)
        {
                    ...
                    loop))
            {
                ...
                /*
                DPCT1023:184: The DPC++ sub-group does not support mask options
            ...
            /*
            DPCT1023:185: The DPC++ sub-group does not support mask options for
    ...
    /*
    DPCT1023:186: The DPC++ sub-group does not support mask options for
    sycl::select_from_group.
    */
    aligned_nodes =
...
void runNeedlemanWunschKernel(uint8_t* nodes,
                                         ...
                                         sycl::nd_item<3> item_ct1)
{
    *aligned_nodes = runNeedlemanWunsch<uint8_t, int16_t, SizeT>(
        nodes, graph, node_id_to_pos, graph_count, incoming_edge_count,
        incoming_edges, outgoing_edge_count, outgoing_edges, read, read_length,
        scores, scores_width, alignment_graph, alignment_read, gap_score,
        mismatch_score, match_score, item_ct1);
}
           ...
           SizeT* aligned_nodes)
{
    dpct::get_default_queue().parallel_for(
        ...
        [=](sycl::nd_item<3> item_ct1) [[intel::reqd_sub_group_size(32)]] {
            runNeedlemanWunschKernel(
                nodes, graph, node_id_to_pos, graph_count, incoming_edge_count,
                incoming_edges, outgoing_edge_count, outgoing_edges, read,
                read_length, scores, scores_width, alignment_graph,
                alignment_read, gap_score, mismatch_score, match_score,
                aligned_nodes, item_ct1);
        });
    /*
