...
SizeT getNodeIDToMSAPosDevice(SizeT node_count,
                                         SizeT* sorted_poa,
...
template <typename SizeT>
void generateMSADevice(uint8_t* nodes,
                                  ...
                                  sycl::nd_item<3> item_ct1)
{
    ...
    uint16_t s = item_ct1.get_local_id(2);
    if (s >= num_sequences)
        return;
...
template <typename SizeT>
void generateMSAKernel(uint8_t* nodes_d,
                                  uint8_t* consensus_d,
                                  ...
                                  SizeT *msa_length)
{
    ...
    uint32_t window_idx = item_ct1.get_group(2);
    ...
    if (item_ct1.get_local_id(2) ==
        0) // one thread will fill in node_id_to_msa_pos and msa_length for it's
           // block/window
    {
        ...
        *msa_length = getNodeIDToMSAPosDevice<SizeT>(
            sequence_lengths[0], sorted_poa, node_id_to_msa_pos,
            node_alignment_counts);
    ...
    */
    ...
    generateMSADevice<SizeT>(
        nodes, num_sequences, outgoing_edge_count, outgoing_edges,
        outgoing_edges_coverage, outgoing_edges_coverage_count,
        node_id_to_msa_pos, sequence_begin_nodes_ids,
        multiple_sequence_alignments, *msa_length, max_sequences_per_poa,
        max_limit_consensus_size, item_ct1);
}
