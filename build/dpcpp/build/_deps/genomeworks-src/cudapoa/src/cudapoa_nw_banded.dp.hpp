...
SizeT get_band_start_for_row(SizeT row_idx, float gradient, SizeT band_width, SizeT max_column)
{

    SizeT start_pos = SizeT(row_idx * gradient) - band_width / 2;

    start_pos = sycl::max(start_pos, 0);
...
ScoreT* get_score_ptr(ScoreT* scores, SizeT row, SizeT column, SizeT band_start, SizeT band_width)
{
...
template <typename ScoreT, typename SizeT>
void set_score(ScoreT* scores, SizeT row, SizeT column, ScoreT value, float gradient, SizeT band_width, SizeT max_column)
{
...
template <typename ScoreT, typename SizeT>
void initialize_band(ScoreT* scores, SizeT row, ScoreT value, float gradient, SizeT band_width, SizeT max_column,
...
template <typename ScoreT, typename SizeT>
ScoreT get_score(ScoreT* scores, SizeT row, SizeT column, float gradient, SizeT band_width, SizeT max_column, const ScoreT min_score_value)
{
    SizeT band_start = get_band_start_for_row(row, gradient, band_width, max_column);
    SizeT band_end   = band_start + band_width;
...
template <typename ScoreT, typename SizeT>
ScoreT4<ScoreT> get_scores(ScoreT* scores,
                                      SizeT node,
                                      SizeT read_pos,
                                      float gradient,
                                      SizeT band_width,
                             ...
                             sycl::nd_item<3> item_ct1)
{
    ...
    const ScoreT min_score_value =
        2 * sycl::abs(
                sycl::min(sycl::min(gap_score, mismatch_score), -match_score) -
                1) +
        score_type_min_limit;
        ...
        for (SizeT graph_pos = 0; graph_pos < graph_count; graph_pos++)
        {
            ...
            else
            {
                ...
                for (uint16_t p = 0; p < pred_count; p++)
                {
                    ...
                    penalty = sycl::max(
                        penalty, get_score(scores, pred_node_graph_pos,
                                           SizeT{0}, gradient, band_width,
    ...
    for (SizeT graph_pos = 0; graph_pos < graph_count; graph_pos++)
    {
        ...
        initialize_band(scores, score_gIdx, min_score_value, gradient,
                        band_width, max_column, item_ct1);
        ...
        for (SizeT read_pos = lane_idx * CELLS_PER_THREAD + band_start; read_pos < band_start + band_width; read_pos += WARP_SIZE * CELLS_PER_THREAD)
        {
                    ...
                    loop))
            {
                ...
                /*
                DPCT1023:187: The DPC++ sub-group does not support mask options
            ...
            /*
            DPCT1023:188: The DPC++ sub-group does not support mask options for
    ...
    /*
    DPCT1023:189: The DPC++ sub-group does not support mask options for
    sycl::select_from_group.
    */
    aligned_nodes =
