                                  ...
                                  uint32_t max_limit_consensus_size)
{
    ...
    while (predecessors[max_score_id] != -1)
    {
        ...
        consensus_pos = sycl::min((unsigned int)(consensus_pos + 1),
                                  (unsigned int)(max_limit_consensus_size - 1));
        consensus_count++;
    }
    consensus[consensus_pos] = nodes[max_score_id];
    uint16_t cov             = node_coverage_counts[max_score_id];
...
template <typename SizeT>
void generateConsensusKernel(uint8_t* consensus_d,
                                        uint16_t* coverage_d,
                                        ...
                                        sycl::nd_item<3> item_ct1)
{
    ...
    int32_t window_idx =
        item_ct1.get_group(2) * CUDAPOA_MAX_CONSENSUS_PER_BLOCK +
...
template <typename SizeT>
void generateConsensusTestKernel(uint8_t* nodes,
                                            SizeT node_count,
                                            SizeT* graph,
                                            SizeT* node_id_to_pos,
                                            SizeT* incoming_edges,
                               ...
                               uint32_t max_limit_consensus_size)
{
    dpct::get_default_queue().parallel_for(
        ...
        [=](sycl::nd_item<3> item_ct1) {
            generateConsensusTestKernel<SizeT>(
                nodes, node_count, graph, node_id_to_pos, incoming_edges,
                incoming_edge_count, outgoing_edges, outgoing_edge_count,
                incoming_edge_w, predecessors, scores, consensus, coverage,
                node_coverage_counts, node_alignments, node_alignment_count,
                max_limit_consensus_size);
        });
    /*
