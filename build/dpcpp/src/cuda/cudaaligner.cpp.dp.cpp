    ...
    , stream_(0)
{
    ...
    /*
    DPCT1003:150: Migrated API does not return error code. (*, 0) is inserted.
    You may need to rewrite this code.
    */
    GW_CU_CHECK_ERR((dpct::dev_mgr::instance().select_device(device_id), 0));

    /*
    DPCT1003:151: Migrated API does not return error code. (*, 0) is inserted.
    You may need to rewrite this code.
    */
    GW_CU_CHECK_ERR((stream_ = dpct::get_current_device().create_queue(), 0));
...
CUDABatchAligner::~CUDABatchAligner()
{
    aligner_.reset();
    /*
    DPCT1003:152: Migrated API does not return error code. (*, 0) is inserted.
    You may need to rewrite this code.
    */
    GW_CU_CHECK_ERR((dpct::get_current_device().destroy_queue(stream_), 0));
}
