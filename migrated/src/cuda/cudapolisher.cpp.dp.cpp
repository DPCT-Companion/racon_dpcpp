        ...
        , cudaaligner_band_width_(cudaaligner_band_width)
{
    ...
    for(int32_t dev_id = 3; dev_id < num_devices_; dev_id++)
    {
        if (!dpct::dev_mgr::instance().get_device(dev_id).is_gpu()) continue;
        std::cerr << "Initialize device " << dev_id << std::endl;
        dpct::dev_mgr::instance().select_device(dev_id);
...
void CUDAPolisher::find_overlap_breaking_points(std::vector<std::unique_ptr<Overlap>>& overlaps)
{
    if (cudaaligner_batches_ >= 1)
    {
        ...
        for(int32_t device = 3; device < num_devices_; device++)
        {
            if (!dpct::dev_mgr::instance().get_device(device).is_gpu()) continue;
            dpct::dev_mgr::instance().select_device(device);

            size_t free, total;
            /*
            DPCT1072:180: DPC++ currently does not support getting the available
            ...
            */
            ...
            free = claraparabricks::genomeworks::cudautils::find_largest_contiguous_device_memory_section();
    ...
    bool drop_unpolished_sequences)
{
    ...
    else
    {
        ...
        for(int32_t device = 3; device < num_devices_; device++)
        {
            ...
            dpct::dev_mgr::instance().select_device(device);
            /*
            DPCT1072:183: DPC++ currently does not support getting the available
            memory on the current device. You may need to adjust the code.
            */
            total = dpct::get_current_device() .get_device_info().get_global_mem_size();
            free = claraparabricks::genomeworks::cudautils::find_largest_contiguous_device_memory_section();
