                                  ...
                                  sycl::nd_item<3> item_ct1)
{
    ...
    nw_score_t myscore = [scores, i, j, p] {
        int k, l;
        std::tuple<int,int> t = to_band_indices(i, j, p);
        k = std::get<0>(t);
        l = std::get<1>(t);
        return scores(k, l);
    }();
                               ...
                               sycl::nd_item<3> item_ct1)
{
    ...
    while (k < scores.num_rows())
    {
        for (int32_t l = 0; l < scores.num_cols(); ++l)
        {
            ...
            int32_t i, j;
            std::tuple<int,int> t = to_matrix_indices(k, l, p);
            i = std::get<0>(t);
            j = std::get<1>(t);
    ...
    int32_t n_alignments, int32_t p, sycl::queue *stream)
{
    ...
    /*
    DPCT1049:193: The workgroup size passed to the SYCL kernel may exceed the
    limit. To get the device limit, query info::device::max_work_group_size.
    Adjust the workgroup size if needed.
    */
    std::cout << "work group size for 193: compute_blockdims" << compute_blockdims[2] << ", blocks " << blocks[1] <<std::endl;
