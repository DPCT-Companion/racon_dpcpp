...
{
    InternalData(const memory_distribution mem,
                 const int32_t n_alignments_initial,
                 const DefaultDeviceAllocator &allocator, sycl::queue *stream)
        : seq_h(2 * mem.sequence_memory / sizeof(char)), seq_starts_h(),
          results_h(mem.results_memory / sizeof(char)), result_lengths_h(),
          result_starts_h(),
          ...
          query_patterns(mem.query_patterns_memory / sizeof(WordType),
                         allocator, stream)
    {
...
AlignerGlobalMyersBanded::AlignerGlobalMyersBanded(
    int64_t max_device_memory, int32_t max_bandwidth,
    DefaultDeviceAllocator allocator, sycl::queue *stream, int32_t device_id)
    : data_(), stream_(stream), device_id_(device_id),
      max_bandwidth_(max_bandwidth), alignments_()
{
