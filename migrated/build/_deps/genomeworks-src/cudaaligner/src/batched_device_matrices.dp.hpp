...
inline bool error(int32_t x, int32_t y, const sycl::stream &stream_ct1)
{
stream_ct1 << "false";
    return false;
}
...
public:
    ...
    device_matrix_view(T* storage, int32_t n_rows, int32_t n_cols)
        : data_(storage)
        , n_rows_(n_rows)
        , n_cols_(n_cols)
    {
...
public:
    ...
    public:
        ...
        device_matrix_view<T> get_matrix_view(int32_t id, int32_t n_rows, int32_t n_cols)
        {
            assert(storage_ != nullptr);
        ...
        inline int32_t get_max_elements_per_matrix(int32_t id) const
        {
            assert(id < n_matrices_);
    ...
    batched_device_matrices(int64_t max_elements,
                            DefaultDeviceAllocator allocator,
                            sycl::queue *stream)
        : storage_(max_elements, allocator, stream),
          offsets_(allocator, stream), dev_(1, allocator, stream),
          offsets_host_(1, 0)
    {
    }

    batched_device_matrices(int32_t n_matrices, int32_t max_elements_per_matrix,
                            DefaultDeviceAllocator allocator,
                            sycl::queue *stream)
        : storage_(static_cast<size_t>(n_matrices) *
                       static_cast<size_t>(max_elements_per_matrix),
                   allocator, stream),
          offsets_(n_matrices + 1, allocator, stream),
          dev_(1, allocator, stream), offsets_host_(n_matrices + 1)
    {
        assert(n_matrices >= 0);
        assert(max_elements_per_matrix >= 0);
        stream->memset(storage_.data(), 0, storage_.size() * sizeof(T));
    ...
    void construct_device_matrices_async(sycl::queue *stream)
    {
        assert(offsets_host_.size() >= 1);
    ...
    matrix<T> get_matrix(int32_t id, int32_t n_rows, int32_t n_cols,
                         sycl::queue *stream)
    {
        assert(id >= 0);
        assert(n_rows >= 0);
