...
public:
        ...
        , output_mask_(output_mask)
    {
        ...
        // Allocate.
        block_data_h_ = (uint8_t *)sycl::malloc_host( total_h_, dpct::get_default_queue());
        dpct::get_default_queue().memset(block_data_h_, 0, total_h_).wait();
        block_data_d_ = (uint8_t *)sycl::malloc_device( total_d_, dpct::get_default_queue());
        dpct::get_default_queue().memset(block_data_d_, 0, total_d_).wait();
    }

    ~BatchBlock()
    {
        sycl::free(block_data_d_, dpct::get_default_queue());
        sycl::free(block_data_h_, dpct::get_default_queue());
    }
                                     ...
                                     int32_t mismatch_score, int32_t gap_score, int32_t match_score)
    {
        ...
        */
        ...
        free = cudautils::find_largest_contiguous_device_memory_section();
