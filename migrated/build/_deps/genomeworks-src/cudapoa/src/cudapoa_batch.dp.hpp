...
public:
    ...
    void generate_poa()
    {
        ...
        //Copy sequencecs, sequence lengths and window details to device
         stream_->memcpy(input_details_d_->sequences, input_details_h_->sequences,num_nucleotides_copied_ * sizeof(*input_details_h_->sequences));
        stream_->memcpy(input_details_d_->base_weights,input_details_h_->base_weights,num_nucleotides_copied_ *sizeof(*input_details_h_->base_weights));
        stream_->memcpy(input_details_d_->window_details,input_details_h_->window_details,poa_count_ *sizeof(*input_details_h_->window_details));
        stream_->memcpy(input_details_d_->sequence_lengths,input_details_h_->sequence_lengths,global_sequence_idx_ *sizeof(*input_details_h_->sequence_lengths));
                             ...
                             std::vector<genomeworks::cudapoa::StatusType>& output_status)
    {
        ...
        print_batch_debug_message(msg);
        stream_->memcpy(output_details_h_->consensus,output_details_d_->consensus,batch_size_.max_consensus_size * max_poas_ *sizeof(*output_details_h_->consensus));
        stream_->memcpy(output_details_h_->coverage,output_details_d_->coverage,batch_size_.max_consensus_size * max_poas_ *sizeof(*output_details_h_->coverage));
        stream_->wait();
                       ...
                       std::vector<StatusType>& output_status)
    {
        ...
        stream_->memcpy(output_details_h_->multiple_sequence_alignments,output_details_d_->multiple_sequence_alignments,max_poas_ * max_sequences_per_poa_ *batch_size_.max_consensus_size *sizeof(*output_details_h_->multiple_sequence_alignments));
                    ...
                    std::vector<StatusType>& output_status)
    {
        ...
        stream_->memcpy(graph_details_h_->nodes, graph_details_d_->nodes,sizeof(*graph_details_h_->nodes) *max_nodes_per_window_ * max_poas_);

        stream_->memcpy(graph_details_h_->incoming_edges,graph_details_d_->incoming_edges,sizeof(*graph_details_h_->incoming_edges) *max_nodes_per_window_ *CUDAPOA_MAX_NODE_EDGES * max_poas_);

        stream_->memcpy(graph_details_h_->incoming_edge_weights,graph_details_d_->incoming_edge_weights,sizeof(*graph_details_h_->incoming_edge_weights) *max_nodes_per_window_ *CUDAPOA_MAX_NODE_EDGES * max_poas_);

        stream_->memcpy(graph_details_h_->incoming_edge_count,graph_details_d_->incoming_edge_count,sizeof(*graph_details_h_->incoming_edge_count) *max_nodes_per_window_ * max_poas_);

        stream_->memcpy(input_details_h_->sequence_lengths,input_details_d_->sequence_lengths,global_sequence_idx_ *sizeof(*input_details_h_->sequence_lengths));
