...
warp_error = sycl::select_from_group(item_ct1.get_sub_group(), warp_error, 0);        if (warp_error)
        {
            return;
        }
    ...
    int8_t output_mask, const BatchConfig &batch_size)
{
    ...
    std::cout << total_windows << std::endl;
    std::cout << nblocks << std::endl;
    std::cout << nwindows_per_block << std::endl;
    std::cout << consensus_num_blocks << std::endl;
