...
#include <claraparabricks/genomeworks/utils/limits.dp.hpp>
                              ...
                              sycl::nd_item<3> item_ct1)
{
    ...
    SizeT band_end = band_start + band_width;
    band_start = sycl::max((SizeT)1, band_start);
                                     ...
                                     sycl::nd_item<3> item_ct1)
{
    ...
    band_width = sycl::min(band_width, (SizeT)1536);
...
ScoreT last_score = sycl::shift_group_right( item_ct1.get_sub_group(), score.s3, 1);                if (lane_idx == 0)
                {
                    last_score = first_element_prev_score;
                }

                ScoreT tscore =
                    sycl::max((ScoreT)(score.s2 + gap_score), score.s3);
                if (tscore > score.s3)
                {
                    score.s3 = tscore;
                    loop     = true;
                ...
                tscore = sycl::max((ScoreT)(score.s1 + gap_score), score.s2);
                if (tscore > score.s2)
                {
                    score.s2 = tscore;
                    loop     = true;
                ...
                tscore = sycl::max((ScoreT)(score.s0 + gap_score), score.s1);
                if (tscore > score.s1)
                {
                    score.s1 = tscore;
                    loop     = true;
                ...
                tscore = sycl::max((ScoreT)(last_score + gap_score), score.s0);
                if (tscore > score.s0)
                {
                    score.s0 = tscore;
                    loop     = true;
...
first_element_prev_score = sycl::select_from_group( item_ct1.get_sub_group(), score.s3, WARP_SIZE - 1);
...
aligned_nodes = sycl::select_from_group(item_ct1.get_sub_group(), aligned_nodes, 0);    return aligned_nodes;
}
