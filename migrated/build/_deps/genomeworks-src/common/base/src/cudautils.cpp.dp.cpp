...
std::size_t find_largest_contiguous_device_memory_section() try {
    ...
    */
    ...
    dpct::dev_mgr::instance().get_device(3).get_device_info(prop);
    total = prop.get_global_mem_size();
    std::cout << total << std::endl;
    std::cout << prop.get_major_version() << std::endl;
    ...
    while (true)
    {
        ...
        dummy_ptr = (void *)sycl::malloc_device( size_to_try, dpct::get_default_queue());        // if it was able to allocate memory free the memory and return the size
        if (dummy_ptr)
        {
            sycl::free(dummy_ptr, dpct::get_default_queue());
            std::cout << "Free m " << size_to_try << std::endl;
            return size_to_try;
        }
