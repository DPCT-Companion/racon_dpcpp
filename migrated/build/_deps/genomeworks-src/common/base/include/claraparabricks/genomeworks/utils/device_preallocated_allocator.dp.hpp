...
public:
    ...
    int
    DeviceAllocate(void **ptr, size_t bytes_needed,
                   const std::vector<sycl::queue *> &associated_streams) try {
...
private:
    ...
    struct MemoryBlock
    {
        ...
        std::vector<sycl::queue *> associated_streams;
    };

    /// \brief allocates the underlying buffer
    /// \param buffer_size
    /// \return allocated shared_ptr
    static std::unique_ptr<char, void (*)(char *)>
    create_buffer(size_t buffer_size) try {
        ...
        dpct::get_default_queue().memset(ptr, 0, buffer_size).wait();
    ...
    /// \return cudaSuccess if allocation was successful, cudaErrorMemoryAllocation otherwise
    int
    allocate_memory_block(void **ptr, size_t bytes_needed,
                          const std::vector<sycl::queue *> &associated_streams)
    {
        *ptr = nullptr;
    ...
    /// \return error status
    int free_memory_block(void *pointer)
    {
        ...
        for (sycl::queue *associated_stream :
             block_to_be_freed_iter->associated_streams)
        {
