...
AlignerGlobal::AlignerGlobal(int32_t max_query_length,
                             int32_t max_target_length, int32_t max_alignments,
                             DefaultDeviceAllocator allocator,
                             sycl::queue *stream, int32_t device_id)
    : max_query_length_(throw_on_negative(
          max_query_length, "max_query_length must be non-negative.")),
      max_target_length_(throw_on_negative(
          max_target_length, "max_target_length must be non-negative.")),
      max_alignments_(throw_on_negative(
          max_alignments, "max_alignments must be non-negative.")),
      alignments_(),
      sequences_h_(2 * std::max(max_query_length, max_target_length) *
                   max_alignments),
      sequence_lengths_h_(2 * max_alignments),
      results_h_(calc_max_result_length(max_query_length, max_target_length) *
                 max_alignments),
      result_lengths_h_(max_alignments), stream_(stream), device_id_(device_id)
{
    if (max_alignments < 1)
    {
    ...
    stream->memset(sequences_d_.data(), 0, sizeof(char) * sequences_d_.size());
    stream->memset(sequence_lengths_d_.data(), 0, sizeof(char) * sequence_lengths_d_.size());
    stream->memset(results_d_.data(), 0, sizeof(char) * results_d_.size());
    stream->memset(result_lengths_d_.data(), 0, sizeof(char) * result_lengths_d_.size());
}
...
StatusType AlignerGlobal::align_all()
{
    ...
    stream_->memcpy(sequence_lengths_d_.data(), sequence_lengths_h_.data(),2 * sizeof(int32_t) * num_alignments);
    stream_->memcpy(sequences_d_.data(), sequences_h_.data(), 2 * sizeof(char) * max_alignment_length *num_alignments);
    ...
    stream_->memcpy(results_h_.data(), results_d_.data(),sizeof(int8_t) * max_result_length * num_alignments);
    stream_->memcpy(result_lengths_h_.data(), result_lengths_d_.data(),sizeof(int32_t) * num_alignments);
    return StatusType::success;
}
